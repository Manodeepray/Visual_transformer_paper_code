...
class VIT(Model):
    def __init__(self , N_HEAD , HIDDEN_SIZE , N_PATCHES ,    N_LAYERS , N_DENSE_UNITS ):
        ...
        self.transformer_encoder_layer_1 = [Transformer_encoder(N_HEAD , HIDDEN_SIZE) for _ in range(N_LAYERS)]
        self.transformer_encoder_layer_2 = [Transformer_encoder(N_HEAD , HIDDEN_SIZE) for _ in range(N_LAYERS)]
    ...
    def call(self , inputs):
        ...
        for i in range(self.N_LAYERS):
            x = self.transformer_encoder_layer_1[i](x)
            
        x = self.TokenLearner(x)

            
        for i in range(self.N_LAYERS):
            x = self.transformer_encoder_layer_2[i](x)
        x = Flatten()(x)
